...
// initialized to -1 so we recognize if it was already initialized and initialize the last_x/y
...
void setColor(int red, int green, int blue){
   if (trueColor){
         colour.pixel = ((blue & 0xff) | ((green & 0xff) << 8) | ((red & 0xff) << 16));
    }
   else 
          printf(stderr, "Need to allocate color, but not here\n");
...
int BiInterpol(int current_y, int current_x)
	{	
